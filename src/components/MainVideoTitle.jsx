...
const MainVideoTitle = ({title, overview}) => {
  ...
  return (
    <div className="absolute inset-0 px-20 flex flex-col justify-center bg-linear-to-r from-black text-white">
        <h1 className='text-6xl font-bold p-2'>{title}</h1>
        <p className='p-3 w-3/12'>{overview}</p>
        <div className='flex gap-5'>
            <button className='py-3 px-8 w-40 bg-gray-500 hover:opacity-80 text-lg rounded-3xl cursor-pointer'> 
            ...
            <button className='py-3 px-8 w-40 bg-gray-500 hover:opacity-80 text-lg rounded-3xl cursor-pointer'>More Info</button>
        </div>
    </div>
  )
}
